...
map.bound_xyz = boundary;  %1x6 matrix
map.blocks = blocks;
map.margin = margin;
...
for i = 1:size(blocks,1)
  ijk_min = pos2sub(map, blocks(i,1:3) - margin*ones(1,3));  %ijk_min: 1x3 matrix, obstacle min index
  ijk_min(ijk_min < 1) = 1;
  ijk_min(ijk_min > map_sz) = map_sz(ijk_min > map_sz);  %limit the obstacle index in the boundary index

  ijk_max = pos2sub(map, blocks(i,4:6) + margin*ones(1,3));  %ijk_max: 1x3 matrix, obstacle max index
  ijk_max(ijk_max < 1) = 1;
  ijk_max(ijk_max > map_sz) = map_sz(ijk_max > map_sz);  %limit the obstacle index in the boundary index

  [I, J, K] = meshgrid(ijk_min(1):ijk_max(1), ijk_min(2):ijk_max(2),...
                     ijk_min(3):ijk_max(3));
  map.occgrid(sub2ind(map_sz,I(:),J(:),K(:))) = 1;  %set obstacle mesh as 1, sub2ind see pos2ind.m
end
%now map.occgrid is the map we need
end